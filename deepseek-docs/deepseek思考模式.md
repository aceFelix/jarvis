...
**控制参数（OpenAI 格式）控制参数（Anthropic 格式）控制参数（Responses API 格式）思考模式开关(1)`{"thinking": {"type": "enabled/disabled"}}``{"reasoning": {"effort": "none/low/high/max"}}`
(none 表示关闭思考模式)思考强度控制(2)`{"reasoning_effort": "low/high/max"}``{"output_config": {"effort": "low/high/max"}}`**
