...
- 13 条桌面指令：`message` / `sessions.{list,open,new}` / `models.{list,select}` / `voices.{list,select}` / `metrics.get` / `state.get` / `answer_user` / `talk.{start,stop}`。
...
| 前后端协议漂移 | protocol.py 为唯一来源 + contracts.ts 镜像；测试校验 13 指令全注册 |
