...
- **桌面入口** — 双击桌面图标直接打开实时语音窗口，热键召唤（新 GUI 开发中）
...
```bash
jarvis              # 进入 REPL 对话
jarvis --talk       # 直接启动实时语音对话窗口（桌面图标同款入口）
...
| 操作 | 效果 |
|------|------|
| `Ctrl+C` | **任意阶段中断**（LLM 流式输出中 / 工具执行中 / 思考中均可立即停止），回到输入提示符 |
| `/exit` 或 `/quit` | 退出 REPL |
...
双击桌面「JARVIS」图标 → 直接打开方舟反应炉实时语音窗口：
窗口内直接说话即可对话（可打断），点「结束」/按 ESC/说「退下」结束会话，点 X 关闭窗口。

> 📌 原「无窗口后台常驻 + 系统托盘」模式已于 2026-08 下线，由新一代三栏 GUI 工作台（开发中）取代。
> 定时提醒/每日简报/资源监控等主动服务随之进入休眠态，将随新 GUI 一并回归。
> 文本对话请用 `jarvis`（REPL），语音对话用 `/voice`。
...
| 平台 | 行为 |
|------|------|
| Windows | 静默启动（无终端窗口），直接打开实时语音窗口 |
| macOS | Terminal.app 内启动实时语音会话 |
| Linux | 终端内前台运行（终端不能关） |
